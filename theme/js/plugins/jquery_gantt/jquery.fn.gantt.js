    ...
    $.fn.gantt = function (options) {
        ...
        */
        ...
        var core = {
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        entries.push('<div class="gantt-tooltip row name row' + i + (entry.desc ? '' : ' fn-wide') + '" id="rowheader' + i + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '" title="'+(entry.name || '')+'" data-toggle="tooltip">');
                        entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + (entry.startLink || '') + (entry.name || '') + (entry.endLink || '') + '</span>');
                        entries.push('</div>');

                        if (entry.desc) {
                            entries.push('<div class="gantt-tooltip row desc row' + i + ' " id="RowdId_' + i + '" data-id="' + entry.id + '" title="'+(entry.desc || '')+'" data-toggle="tooltip">');
            ...
            // the entire chart
            createProgressBar: function (days, cls, desc, label, dataObj, name, ppTitle, ppDescription) {
                var cellWidth = tools.getCellSize();
                var barMarg = tools.getProgressBarMargin() || 0;
                var bar = $('<div class="bar" data-container="body" data-title="<b>'+(ppTitle ? ppTitle : 'Title')+'</b>" data-html=true data-placement="auto right" data-content="'+(ppDescription ? ppDescription : 'Description')+'"><div class="fn-label">' + label + '</div></div>')
                        .addClass(cls)
                        .css({
                            ...
                            switch (settings.scale) {
                                ...
                                case "hours":
                                    ...
                                    _bar = core.createProgressBar(
                                                ...
                                                day.ppTitle ? day.ppTitle : "",
                                                day.ppDescription ? day.ppDescription : ""
                                            );
