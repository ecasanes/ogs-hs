...
class Case_File extends MY_Controller {
	...
	public function __construct() {
		...
		$step_titles = array(
			'Create Basic Case File',
			'Defining the Problem',
			'Build Equipment History',
	...
	public function view( $id, $action = '' ) {
		...
		$equipment_history_questions = $document_model->get_all_equipment_history_questions_with_answer( $document_id );
		$equipment_history_categories = $document_model->get_equipment_history_categories();
		...
		foreach ( $equipment_history_categories as $category ) {
			...
			$equipment_history_questions_array[$counter] = $document_model->get_equipment_history_questions_by_category( $category_id );
		...
		$model_data['failure_mechanism_subdivision_value'] = $this->get_dropdown_subcategory( $failure_mechanism_subdivision, 'failure_mechanism', 'subcategory', 'name' );
		...
		$this->load->view( 'view/view-casefile', $model_data );
		if ( $action == 'rank' ) {
		...
		if ( $action == 'update' ) {
			...
			if ( $update_flag ) {
				...
				foreach ( $equipment_history_question_id as $question_id ) {
					...
					$document_model->update_answer( $model_id, $answer_id, $answer, '', '', '', '', '' );
	...
	public function _save_problem( $id, $action ) {
		...
		if ( $action == 'update' ) {
			...
			if ( $update_flag ) {
				...
				$new_date_array[] = convert_string_to_date( $timeline_date );

				/*$date_counter = 0;
				foreach($month as $month_value){
					$single_date = $month_value.'/'.$day[$date_counter].'/'.$year[$date_counter];
					$new_date_array[] = convert_string_to_date($single_date);
					$date_counter++;
				}*/
				//END DATE CONVERSION


				$result = $main_model->update_step_2( $form_id, $summary, $detection_notification, $detection_date, $detection_details, $failure_notification, $failure_description );
	...
	public function _save_solution( $id, $action ) {
		...
		if ( $action == 'update' ) {
			...
			if ( $update_flag ) {
				...
				$main_model->update_step_4( $form_id, $summary, $risks );

				$document_model->update_simple_cost_breakdown( $model_id, $cost_item, $cost_description );
				$document_model->update_simple_benefits_breakdown( $model_id, $benefit_item, $benefit_description );
	...
	public function _edit_casefile( $id, $step = 1, $warning_status = '' ) {
		...
		$header_data['current_page_name'] = 'Level 2 - Forensic DECF';

		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-'.$step.'', $model_data );
		$this->load->view( 'layout/footer' );
	}
	...
	public function _edit_equipment_history( $id, $step ) {
		...
		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-3', $model_data );
		$this->load->view( 'layout/footer' );
	}
	...
	public function _edit_problem( $id, $step ) {
		...
		foreach ( $failure_impacts as $impact ) {
			...
			$failure_impact_array[$impact_counter]['area_of_impact'] = $this->get_dropdown_menu( $impact->area_of_impact_id, 'area_of_impact' );
		...
		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-2', $model_data );
		$this->load->view( 'layout/footer' );
	}
	...
	public function _edit_root_cause( $id, $step ) {
		...
		$model_data['failure_mechanism'] = $this->get_dropdown_menu( $failure_mechanism, 'failure_mechanism' );
		...
		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-4', $model_data );
		$this->load->view( 'layout/footer' );
	}
	...
	public function _edit_solution( $id, $step ) {
		...
		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-5', $model_data );
		$this->load->view( 'layout/footer' );
	}
	...
	public function _edit_process( $id, $step ) {
		...
		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-6', $model_data );
		$this->load->view( 'layout/footer' );
	}
	...
	public function _edit_results( $id, $step ) {
		...
		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-7', $model_data );
		$this->load->view( 'layout/footer' );
	}
	...
	public function _edit_requirements( $id, $step ) {
		...
		$findings = $document_details->findings;
		$summary = $document_details->share_summary;
		$detection = $document_details->detection;
		$prevention = $document_details->prevention;
		$recommendations = $document_details->recommendations;
		...
		$this->load->view( 'layout/header', $header_data );
		$this->load->view( 'case_file/step-8', $model_data );
		$this->load->view( 'layout/footer' );
	}
...
/* End of file case_file.php */
/* Location: ./application/controllers/case_file.php */
